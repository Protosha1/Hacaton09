...
async def start_negotiation(
    ...
    """Start a new session. user_id is always taken from the token (FR-19)."""
    service = NegotiationService(db)
    return await service.start_session(
        scenario_id=req.scenario_id,
        user_id=user.id,                       # <- из токена, не из тела
...
async def send_message(
    ...
    return await service.process_message(
        req.session_id,
        req.message,
        current_user_id=user.id,               # <- ownership check
    )
...
async def send_voice_message(
    ...
    user_text = await VoiceService.speech_to_text(audio_bytes)
...
async def end_negotiation(
    ...
    if session.status == SessionStatus.INTERRUPTED:
        raise HTTPException(
            400,
            detail="Cannot finish an interrupted session. Start a new one.",
        )
    ...
    if not messages:
        raise HTTPException(400, detail="No messages in this session")
