...
async def register_admin(
    ...
    # Gate 2: optional secret code
    if settings.ADMIN_REGISTRATION_CODE:
        provided = payload.registration_code or ""
        if not secrets.compare_digest(provided, settings.ADMIN_REGISTRATION_CODE):
            raise HTTPException(403, "Invalid admin registration code.")

    # Gate 3: rate limit per IP
    client_ip = request.client.host if request.client else "unknown"
    if not admin_register_limiter.check(client_ip):
        raise HTTPException(
            429,
            "Too many admin registrations from this IP. Try again later.",
        )
