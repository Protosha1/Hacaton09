...
def _reset_rate_limiter():
    ...
    Otherwise state leaks across tests, and later tests get 429.
    """
...
async def seeded_db(test_db):
    """Active, onboarded user + one ready scenario."""
    user = User(
        id="user-1",
...
async def user_token(client):
    """Register a user AND complete onboarding. Returns JWT."""
    ...
    await client.post(
        ...
        json={"directions": ["management", "clients"], "experience_level": "beginner"},
    )
    return token
...
def mock_llm(monkeypatch):
    ...
    monkeypatch.setattr(llm_service.LLMService, "generate_response", fake_generate)
...
def mock_analysis(monkeypatch):
    ...
    monkeypatch.setattr(analysis_service.AnalysisService, "analyze_session", fake_analyze)